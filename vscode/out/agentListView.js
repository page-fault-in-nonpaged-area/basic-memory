...
exports.AgentListViewProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
class AgentListViewProvider {
    extensionUri;
    workspaceRoot;
    static viewType = 'agentList';
    _view;
    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.type) {
                ...
                case 'openFile':
                    try {
                        ...
                        if (!fs.existsSync(actualPath)) {
                            ...
                            if (fs.existsSync(dir)) {
                                ...
                                const match = files.find(f => f.replace(/'/g, '') === basename);
                                if (match) {
                    ...
                    break;
                case 'deleteMemory':
                    const confirm = await vscode.window.showWarningMessage(`Delete memory "${msg.filename}"?`, { modal: true }, 'Delete');
                    if (confirm === 'Delete') {
                        try {
        ...
        const agentsPattern = new vscode.RelativePattern(this.workspaceRoot, '.github/agents/**/*');
        ...
        const memoryPattern = new vscode.RelativePattern(this.workspaceRoot, '.agent-projects/**/*');
    ...
    toggleAgentPause(agentName) {
        ...
        this.sendAgentData();
        vscode.window.showInformationMessage(`Agent "${agentName}" memory ${current.paused ? 'paused' : 'resumed'}`);
    }
    toggleAgentEnabled(agentName) {
        const controls = this.readAgentControls();
        const current = controls.agents[agentName] ?? { enabled: true, paused: false };
        ...
        this.writeAgentControls(controls);
        this.sendAgentData();
        vscode.window.showInformationMessage(`Agent "${agentName}" memory ${current.enabled ? 'enabled' : 'disabled'}`);
    }
    ...
    getAgentStats(agentName, memoryBaseDir) {
    ...
    getMemories(agentName, memoryBaseDir) {
        // For configured projects, memoryBaseDir IS the project directory
        // For default layout, we need to append the agent name
    ...
    scanMemoriesRecursive(baseDir, dir, memories) {
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
            if (entry.name.startsWith('.'))
                continue;
            if (entry.name === '_immediate.md')
                continue; // shown as permanent UI entry
    ...
    async openImmediateMemory(agentName) {
        const configuredProjects = this.getConfiguredProjects();
        const defaultMemoryBaseDir = path.join(this.workspaceRoot, '.agent-projects');
        const memoryBaseDir = configuredProjects.get(agentName) || defaultMemoryBaseDir;
    ...
    async createNewMemory(agentName) {
...
exports.AgentListViewProvider = AgentListViewProvider;
